...
function attachAdminDiscoveryHandlers(container) {
  ...
  container.querySelectorAll('.disc-set-player').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      // Find the select in the same .admin-discovery-controls parent
      const controls = btn.closest('.admin-discovery-controls');
      const select = controls?.querySelector('.discovery-select');
      if (!select) { console.error('Discovery select not found for', cat, id); return; }
      const level = select.value;
      try {
        await setDiscoveryLevel(currentUser.uid, cat, id, level);
        if (!playerDiscovery[cat]) playerDiscovery[cat] = {};
        playerDiscovery[cat][id] = level;
        showNotification(`Set to ${level} for this player`, 'success');
        renderSection();
      } catch (err) {
        console.error('Discovery set failed:', err);
        showNotification('Failed to update discovery: ' + err.message, 'danger');
      }
    });
  });
  ...
  container.querySelectorAll('.disc-set-all').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        console.error('Discovery setAll failed:', err);
        showNotification('Failed to update discovery: ' + err.message, 'danger');
      }
    });
  });
